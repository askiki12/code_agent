...
- 记忆工具（session-bound `Tool` 子类）：`RememberTool`（`remember`：`content` + 可选 `tags`，写一条记忆，返回 `remembered: <id>`）/ `RecallTool`（`recall`：`query` + 可选 `top_k`（默认 3，clamp 1..10），关键词相关度打分召回，命中 bump usage_count 并落盘，无命中返回 `(no relevant memories)`）在 `memory=True` 时注册；`CreateSkillTool`（`create_skill`：`name`/`description`/`content`，经 `SkillRegistry.add` 写项目级 SKILL.md 复用技能机制，name 非法返回 `invalid skill name`）仅 `memory=True` **且**技能库非空（`skills` 配置）时注册。`memory=False` 时三者不注册（unknown tool）。
...
- `self._system_prompt` 按能力条件组装（`AgentSession.__init__`，ADR-027）：`SYSTEM_PROMPT`（含基础工具选用指引 "Choosing tools" 段）`+ DISPATCH_GUIDANCE`（`allow_subagent=True`）`+ skills_section`（`skills` 非空且扫描到条目时：`Available skills` 列表 + `SKILL_GUIDANCE_USE` + `SKILL_GUIDANCE_CREATE`（`memory=True`））`+ MEMORY_GUIDANCE`（`memory=True`）`+ SUBAGENT_PROMPT_EXTRA`（`allow_subagent=False` 子会话），即提示词与模型实际可见工具保持一致。
