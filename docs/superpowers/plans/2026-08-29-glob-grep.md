...
- Modify: `/home/kiki/workspace/code_agent_project/.agent/03-decisions.md`（ADR-011，工作区根、仓库外，**不入库**，符合 ADR-007）
...
`/home/kiki/workspace/code_agent_project/.agent/03-decisions.md`（工作区根，仓库外）的 `## 后续决策记录处` 之前追加：
...
注意：ADR-011 更新的是工作区根 `.agent/03-decisions.md`（仓库外），本步 git add **不含** `.agent/`（行为规范不入公开仓库，ADR-007）。
