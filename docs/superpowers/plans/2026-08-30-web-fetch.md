...
- Produces:
  ...
  - `is_public_http_url(url: str) -> bool`——scheme 须 http/https；字面 IP 直接判公网（私网拒）；主机名 `localhost`/`.local` 拒绝；配置 HTTP(S) 代理时主机名放行（代理解析真实主机），无代理时 DNS 解析后所有 IP 均须为公网；解析失败返回 False。
